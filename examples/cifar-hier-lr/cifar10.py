...
def parse_args():
    ...
    parser.add_argument('--lr-macro-schedule', type=str)
    parser.add_argument('--lr-micro-schedule', type=str)
    parser.add_argument('--lr-max', type=float, default=0.1)
    parser.add_argument('--save-all-weights', action="store_true")
...
macro = {
    "constant" : HPSchedule.constant(hp_max=args.lr_max),
    "step"     : HPSchedule.step(hp_max=args.lr_max, epochs=args.epochs, breaks=(10, 20), factors=(0.1, 0.1)),
    "linear"   : HPSchedule.linear(hp_max=args.lr_max, epochs=args.epochs),
    "cosine"   : HPSchedule.sgdr(hp_max=args.lr_max, period_length=args.epochs),
}
...
lr_scheduler = HPSchedule.prod_schedule([
    macro[args.lr_macro_schedule],
...
#         model.save('weights/weights-%d' % epoch)

# model.save('weights/final')
